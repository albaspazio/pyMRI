...
class SubjectTransforms:
    ...
    #  -> stdimg, stdimg_brain, stdimg_brain_mask_dil
    #  -> stdimg4, stdimg4_brain, stdimg4_brain_mask_dil
    def transform_epi(self, type='fmri', std_img="", do_bbr=False, wmseg="", logFile=""):
        ...
        if imtest(exfun) is False:
            rrun("fslmaths " + data + " " + os.path.join(folder, "prefiltered_func_data") + " -odt float", logFile=log)
            rrun("fslroi " + os.path.join(folder, "prefiltered_func_data") + " " + exfun + " 100 1", logFile=log)
            rrun("bet2 " + exfun + " " + exfun + " -f 0.3", logFile=log)

            rrun("imrm " + os.path.join(folder, "prefiltered_func_data*"), logFile=log)

            rrun("fslmaths " + exfun + " -bin " + m_exfun, logFile=log)      # create example_function mask (a -thr 0.01/0.1 could have been used to further reduce it)
        ...
        if do_bbr is True:
            ...
            rrun("flirt -ref " + self.subject.t1_brain_data + " -in " + exfun + " -dof 6 -omat " + epi2hr + "_init.mat", logFile=log)
            ...
            if imtest(wmseg) is False:
                ...
                rrun("fast -o " + os.path.join(temp_dir, "temp_" + self.subject.t1_brain_data), logFile=log)
                rrun("fslmaths " + os.path.join(temp_dir, "temp_pve_2") + " -thr 0.5 -bin " + wmseg, logFile=log)
                runsystem("rm -rf " + temp_dir, logFile=log)

            # => epi2hr.mat
            if os.path.isfile(epi2hr + ".mat") is False:
                rrun("flirt -ref " + self.subject.t1_data + " -in " + exfun + " -dof 6 -cost bbr -wmseg " + wmseg + " -init " + epi2hr + "_init.mat" + " -omat " + epi2hr + ".mat" + " -out " + epi2hr + " -schedule " + os.path.join(self.subject.fsl_dir, "etc", "flirtsch", "bbr.sch"), logFile=log)

            # => epi2hr.nii.gz
            if imtest(epi2hr) is False:
                rrun("applywarp -i " + exfun + " -r " + self.subject.t1_data + " -o " + epi2hr + " --premat=" + epi2hr + ".mat" + " --interp=spline", logFile=log)

            runsystem("rm " + epi2hr + "_init.mat", logFile=log)
        else:
            # NOT BBR
            if os.path.isfile(epi2hr + ".mat") is False:
                rrun("flirt -in " + exfun + " -ref " + self.subject.t1_brain_data + " -out " + epi2hr + " -omat " + epi2hr + ".mat" + " -cost corratio -dof 6 -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -interp trilinear", logFile=log)

        if os.path.isfile(hr2epi_mat) is False:
            rrun("convert_xfm -inverse -omat " + hr2epi_mat + " " + epi2hr + ".mat", logFile=log)
        ...
        if os.path.isfile(epi2std + ".mat") is False:
            rrun("convert_xfm -concat " + hr2std + ".mat" + " " + epi2hr + ".mat" + " -omat " + epi2std + ".mat", logFile=log)
        ...
        if imtest(epi2std) is False:
            rrun("flirt -ref " + std_img + " -in " + exfun + " -out " + epi2std + " -applyxfm -init " + epi2std + ".mat" + " -interp trilinear", logFile=log)

        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # EPI <--> STANDARD (non linear)
        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if imtest(epi2std + "_warp") is False:
            rrun("convertwarp --ref=" + std_img_head + " --premat=" + epi2hr + ".mat" + " --warp1=" + hr2std_warp + " --out=" + epi2std + "_warp", logFile=log)

        # invwarp: standard -> highres -> epi
        if imtest(std2epi + "_warp") is False:
            rrun("invwarp -r " + exfun + " -w " + epi2std + "_warp" + " -o " + std2epi + "_warp", logFile=log)
        ...
        if type == "rs":
            if imtest(epi2std4 + "_warp") is False:
                rrun("convertwarp --ref=" + std4_head_img + " --premat=" + epi2hr + ".mat" + " --warp1=" + hr2std4_warp + " --out=" + epi2std4 + "_warp", logFile=log)

            if imtest(std42epi + "_warp") is False:
                rrun("invwarp -r " + exfun + " -w " + epi2std4 + "_warp" + " -o " + std42epi + "_warp", logFile=log)
            ...
            if imtest(os.path.join(self.subject.roi_rs_dir, "t1_wm_rs")) is False:
                rrun("flirt -in " + os.path.join(self.subject.roi_t1_dir, "mask_t1_wm") + " -ref " + self.subject.rs_examplefunc + " -applyxfm -init " + os.path.join(self.subject.roi_rs_dir, "hr2rs.mat") + " -out " + os.path.join(self.subject.roi_rs_dir, "t1_wm_rs"), logFile=log)

            if imtest(os.path.join(self.subject.roi_rs_dir, "t1_csf_rs")) is False:
                rrun("flirt -in " + os.path.join(self.subject.roi_t1_dir, "mask_t1_csf") + " -ref " + self.subject.rs_examplefunc + " -applyxfm -init " + os.path.join(self.subject.roi_rs_dir, "hr2rs.mat") + " -out " + os.path.join(self.subject.roi_rs_dir, "t1_csf_rs"), logFile=log)

            if imtest(os.path.join(self.subject.roi_rs_dir, "t1_gm_rs")) is False:
                rrun("flirt -in " + os.path.join(self.subject.roi_t1_dir, "mask_t1_gm") + " -ref " + self.subject.rs_examplefunc + " -applyxfm -init " + os.path.join(self.subject.roi_rs_dir, "hr2rs.mat") + " -out " + os.path.join(self.subject.roi_rs_dir, "t1_gm_rs"), logFile=log)

            if imtest(os.path.join(self.subject.roi_rs_dir, "t1_brain_rs")) is False:
                rrun("flirt -in " + os.path.join(self.subject.roi_t1_dir, self.subject.t1_brain_data) + " -ref " + self.subject.rs_examplefunc + " -applyxfm -init " + os.path.join(self.subject.roi_rs_dir, "hr2rs.mat") + " -out " + os.path.join(self.subject.roi_rs_dir, "t1_brain_rs"), logFile=log)

            if imtest(os.path.join(self.subject.roi_rs_dir, "t1_brain_mask_rs")) is False:
                rrun("flirt -in " + os.path.join(self.subject.roi_t1_dir, self.subject.t1_brain_data_mask) + " -ref " + self.subject.rs_examplefunc + " -applyxfm -init " + os.path.join(self.subject.roi_rs_dir, "hr2rs.mat") + " -out " + os.path.join(self.subject.roi_rs_dir, "t1_brain_mask_rs"), logFile=log)
    ...
    def transform_dti_t2(self):
        ...
        if imtest(self.subject.dti_nodiff_brain_data) is False:
            rrun("bet " + self.subject.dti_nodiff_data + " " + self.subject.dti_nodiff_brain_data + " -m -f 0.3",
                 logFile=log)  # also creates dti_nodiff_brain_mask_data
        ...
        if os.path.exist(t22hr + ".mat") is False:
            rrun("flirt -in " + self.subject.t2_brain_data + " -ref " + self.subject.t1_brain_data + " -out " + t22hr + " -omat " + t22hr + ".mat" +
                " -bins 256 -cost normmi -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12 -interp trilinear", logFile=log)

        # hr -> t2 linear
        hr2t2 = os.path.join(self.subject.roi_t2_dir, "hr2t2")
        if os.path.exist(hr2t2 + ".mat") is False:
            rrun("convert_xfm -omat " + hr2t2 + ".mat" + " -inverse " + t22hr + ".mat", logFile=log)

        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # DTI <--->  T2 non linear
        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # dti -> t2 linear
        if os.path.exist(self.subject.dti2t2_mat) is False:
            rrun("flirt -in " + self.subject.t2_brain_data + " -ref " + self.subject.dti_nodiff_brain_data + " -omat " + self.subject.dti2t2_mat +
                " -bins 256 -cost normmi -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12 -interp trilinear", logFile=log)

        # t2 -> dti linear
        if os.path.exist(self.subject.t22dti_mat) is False:
            rrun("convert_xfm -omat " + self.subject.t22dti_mat + " -inverse " + self.subject.dti2t2_mat, logFile=log)

        # dti -> t2 non linear
        if imtest(self.subject.dti2t2_warp) is False:
            rrun("fnirt --in=" + self.subject.t2_data + " --ref=" + self.subject.t2_data + " --aff=" + self.subject.dti2t2_mat + " --cout=" + self.subject.dti2t2_warp, logFile=log)

        # t2 -> dti non linear
        if imtest(self.subject.t22dti_warp) is False:
            rrun("invwarp -r " + self.subject.dti_nodiff_data + " -w " + self.subject.dti2t2_warp + " -o " + self.subject.t22dti_warp, logFile=log)

        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # DTI <-- (non-lin) -- t2 -- (lin) -- HIGHRES
        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if imtest(self.subject.dti2hr_warp) is False:
            rrun("convertwarp --ref=" + self.subject.t1_data + " --warp1=" + dti2t2 + "_warp"  + " --postmat=" + t22hr + ".mat" + " --out=" + self.subject.dti2hr_warp, logFile=log)

        if imtest(self.subject.hr2dti_warp) is False:
            rrun("invwarp -r " + self.subject.dti_nodiff_data + " -w " + self.subject.dti2hr_warp + " -o " + hr2dti_warp , logFile=log)
        ...
        if imtest(dti2std + "_warp") is False:
            rrun("convertwarp --ref=" + std_img_head + " --warp1=" + dti2t2 + "_warp" + " --midmat=" + t22hr + ".mat" + " --warp2=" + hr2std_warp + " --out=" + dti2std + "_warp",logFile=log)

        std2dti = os.path.join(self.subject.roi_dti_dir, std_img_label + "2dti")
        if imtest(std2dti + "_warp") is False:
            rrun("invwarp -r " + self.subject.dti_nodiff_data + " -w " + dti2std + "_warp" + " -o " + std2dti + "_warp", logFile=log)

    # DTI <-- (lin) -- HIGHRES -- (non-lin) --> STANDARD
    def transform_dti(self, std_img="", log=None):
        ...
        # -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        if imtest(self.subject.dti_nodiff_data) is False:
            rrun("fslroi " + os.path.join(self.subject.dti_data) + " " + self.subject.dti_nodiff_data + " 0 1", logFile=log)
        if imtest(self.subject.dti_nodiff_brain_data) is False:
            rrun("bet " + self.subject.dti_nodiff_data + " " + self.subject.dti_nodiff_brain_data + " -m -f 0.3", logFile=log)  # also creates dti_nodiff_brain_mask_data
        ...
        if os.path.exist(dti2hr + ".mat") is False:
            rrun("flirt -in " + self.subject.dti_nodiff_data + "_brain" + " -ref " + self.subject.t1_brain_data + " -out " + dti2hr + " -omat " + dti2hr + ".mat" +
                " -bins 256 -cost normmi -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 7 -interp trilinear", logFile=log)

        hr2dti = os.path.join(self.subject.roi_dti_dir, "hr2dti")
        if os.path.exist(hr2dti + ".mat") is False:
            rrun("convert_xfm -omat " + hr2dti + ".mat" + " -inverse " + dti2hr + ".mat", logFile=log)
        ...
        hr2std_warp = os.path.join(self.subject.roi_dir, "reg_" + std_img_label, "hr2" + std_img_label + "_warp")
        if imtest(dti2std + "_warp") is False:
            rrun("convertwarp --ref=" + std_img_head + " --premat=" + dti2hr + ".mat" + " --warp1=" + hr2std_warp + " --out=" + dti2std + "_warp", logFile=log)
    ...
    def transform_roi(self, regtype, pathtype="standard", mask="", orf="", thresh=0.2, islin=True, std_img="", rois=[]):
        ...
        if stdimg == "":
            ...
            # check resolution
            res = str(read_header(std_img, "dx"))
            if "std4" in regtype:
                if res != "4":
    ...
    def transform_nl_hr2epi(self, path_type, roi_name, roi, std_img, epi_label="rs", std_img_label="std"):
        ...
        return transform_l_hr2epi(path_type, roi_name, roi, std_img, epi_label, std_img_label)
    ...
    def transform_nl_std2std4(self, path_type, roi_name, roi, std_img, std_img_label="std"):
        ...
        return transform_l_std2std4(path_type, roi_name, roi, std_img, std_img_label)
