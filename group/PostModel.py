...
class PostModel:
    ...
    def __init__(self, _type:int, regressors:List[Regressor], templ_name:str=None, contrasts:List[Contrast]=None,
        ...
        # check if template name is valid according to the specification applied in ProjectMRI.adapt_batch_files
        ...
        if not os.path.exists(templ_name + ".m"):
            if not os.path.exists(os.path.join(GlobalMRI.get_spm_template_dir(), templ_name + "_job.m")):
