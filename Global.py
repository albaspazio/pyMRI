    ...
    def __init__(self, fsl_ver, ignore_warnings=True):
        ...
        if os.path.isfile(local_settings) is False:
            raise Exception(
                ...
                "copy and rename the file " + os.path.join(self.framework_dir, "examples",
                                                           "local.settings") + " there and modify its content according to your local settings")
        ...
        if self.cat_version.startswith("cat12.7"):
            # cat 12.7
            self.cat_dartel_template = os.path.join(self.spm_dir, "toolbox", self.cat_foldername, "templates_1.50mm",
                                                    "Template_1_IXI555_MNI152.nii")
            self.cat_template_name = "cat27_segment_customizedtemplate_tiv_smooth"
        else:
            # cat 12.8
            self.cat_dartel_template = os.path.join(self.spm_dir, "toolbox", self.cat_foldername,
                                                    "templates_MNI152NLin2009cAsym", "Template_1_Dartel.nii")
            self.cat_shooting_template = os.path.join(self.spm_dir, "toolbox", self.cat_foldername,
        ...
        self.fsl_std_mni_4mm_brain_mask = os.path.join(self.framework_dir, "templates", "images",
                                                       "MNI152_T1_4mm_brain_mask")
        self.fsl_std_mni_4mm_brain_mask_dil = os.path.join(self.framework_dir, "templates", "images",
                                                           "MNI152_T1_4mm_brain_mask_dil")
