...
def check_flirt(omat, inimg, ref,
    ...
    if os.path.exists(omat) is False or overwrite is True:
...
def check_apply_mat(oimg, iimg, mat, ref, overwrite=False, logFile=None):
    if imtest(oimg) is False or overwrite is True:
...
def check_apply_warp(oimg, iimg, warp, ref, overwrite=False, logFile=None):
    if imtest(oimg) is False or overwrite is True:
        if imtest(iimg) is False or imtest(warp) is False or imtest(ref) is False:
            raise Exception(
