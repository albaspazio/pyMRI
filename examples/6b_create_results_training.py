...
from pymri.utility.fslfun import imtest, run, run_notexisting_img
...
if __name__ == "__main__":
    ...
    # init vars
    # ======================================================================================================================
    subjects_list = "subjects56_longitudinal"
    num_cpu = 1
    DO_REGISTER = True
    SESS_ID = 1
    ...
    arr_images_names    = ["R_ATN/2x56_x_age_maskrsn/R_ATN_longitudinal2x56_x_age_maskrsn_tfce_corrp_tstat6", "L_ATN/2x56_maskrsn/L_ATN_longitudinal2x56_maskrsn_tfce_corrp_tstat6"]
    arr_rsn_id          = [10, 8]
    ...
    subjects = project.load_subjects(subjects_list)
    NUM_SUBJ = len(subjects)

    # load melodic template
    with open(template_file_name) as templfile:
    ...
    if DO_REGISTER is True:
        ...
        cnt = 0
        for rsn_id in melodic_template.arr_rsn_id:
            src_res_file    = os.path.join(RSN_DIR, "thresh_zstat" + rsn_id)
            dest_res_file   = os.path.join(RSN_DIR, arr_rsn_labels[cnt])

            if imtest(dest_res_file) is False:
                run("flirt - in " + src_res_file + " -ref " + os.path.join(globaldata.fsl_data_standard, "MNI152_T1_2mm_brain") + " -applyxfm -init " + os.path.join(RESULTS_OUT_DIR, "bg2standard.mat") + " -out " + dest_res_file)
                run("fslmaths " + dest_res_file + " -thr 2.7 " + dest_res_file)
    ...
    for roi in arr_roi:
        ...
        print(filename)
        run("fslmeants -i " + ic_image + " -o " + os.path.join(RESULTS_OUT_DIR, melodic_template.template_name + "_" + filename + ".txt") + " -m " + roi)
