...
class SubjectMRI(Subject):
    """
    MRI-specific Subject class extending the base Subject with MRI infrastructure.
    
    This class adds MRI-specific filesystem structure, image paths, and processing
    capabilities. It represents a subject-session pair in an MRI project with FSL/SPM
    integration and preprocessing pipelines.
    
    Inherits from Subject: label, sessid, project, dir, exist, is_equal, is_in, get_properties
    
    Additional MRI attributes:
        _global (Global): MRI configuration (FSL, SPM, CAT paths)
        transform (SubjectTransforms): Registration and transformation methods
        mpr (SubjectMpr): T1/MPR processing
        dti (SubjectDti): DTI processing
        epi (SubjectEpi): EPI/fMRI/RS processing
    """
